...
**Answer:**
No native deps exist yet. There is no policy covering native modules. The persistence audit ([docs/readiness-audit/08-persistence-save-system.md](08-persistence-save-system.md)) selected IndexedDB / OPFS for browser saves, which avoids `sqlite3`-class native bindings on the client; but if/when the signaling server, AI-gateway, or asset pipeline ([services/](../../services/)) introduces `sharp`, `bcrypt`, `better-sqlite3`, etc., there is no separate native-CVE pipeline (e.g., `cargo audit` for any Rust addons, or `oss-fuzz`-style fuzzing) defined.
...
**Answer:**
No release pipeline exists. No cosign/Sigstore/SLSA workflow, no `.github/workflows/release.yml`, no `keyless-signing.yml`, no `.well-known/cosign.pub`. Pack signing (separate concern) is discussed in [docs/readiness-audit/27-save-tampering-and-pack-signing.md](27-save-tampering-and-pack-signing.md) but **app/release artifact** signing is not.
...
**Answer:**
N/A and undefined. There are no signing keys, no build-time secrets, no dual-control split between "code can build" and "code can sign release." The `pack-signing` audit ([docs/readiness-audit/27-save-tampering-and-pack-signing.md](27-save-tampering-and-pack-signing.md) Q557–Q562) calls out unresolved key custody for **packs**, but **release-key** custody is not even raised. When pack signing or app signing lands, dual-control (build job ≠ signing job, separate workflow with restricted reusable-workflow permissions) is unmodeled.

**Evidence:**
- See [docs/readiness-audit/27-save-tampering-and-pack-signing.md](27-save-tampering-and-pack-signing.md) Q557–Q562
...
10. **Cross-link this audit with [docs/readiness-audit/27-save-tampering-and-pack-signing.md](27-save-tampering-and-pack-signing.md)** so pack-signing key custody and release-signing key custody are designed in one pass, not two.
