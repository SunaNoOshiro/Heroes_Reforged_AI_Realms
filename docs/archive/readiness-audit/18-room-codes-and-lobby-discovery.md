...
**Answer:**
**Yes, by design omission.** With no authentication beyond presenting the code (Q302–Q303 entropy is enumerable, Q310–Q312 rate limits are absent, Q318 host approval is not specified), the first peer to present a valid `JOIN_ROOM` for a code is accepted into WebRTC negotiation. The architecture acknowledges this in `docs/archive/readiness-audit/07-multiplayer.md` ("Room-code only access control … No authentication = trivial griefing.").

**Evidence:**
- `tasks/phase-3/01-multiplayer/01-signaling-server-node-js-websocket-lobby.md`
- `docs/archive/readiness-audit/07-multiplayer.md` Risks section
- `docs/archive/readiness-audit/24-tls-enforcement-and-webrtc-authentication.md` Q475 (acknowledges the gap)
...
**Answer:**
Not applicable — **spectators are not in scope** for M5 multiplayer (per Q140 in `docs/archive/readiness-audit/07-multiplayer.md`). No spectator screen, command, or slot type exists.

**Evidence:**
- `docs/archive/readiness-audit/07-multiplayer.md` Q140
